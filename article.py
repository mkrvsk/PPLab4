...
from dbmodel import Article, Session, User, UpdatedArticle, Moderator
...
@article.route('/api/v1/article/<int:articleId>', methods=['GET'])
def get_article(articleId):
    ...
    # Return user data
    user_data = {'article_id': db_user.article_id, 'name': db_user.name, 'body': db_user.body,
                 'version': db_user.version}
    return jsonify({"user": user_data})


# Delete article by id
@article.route('/api/v1/article/<int:articleId>', methods=['DELETE'])
@auth.login_required
